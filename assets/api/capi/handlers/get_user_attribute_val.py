...
def index_handler(event, context):
    ...
    try:
        ...
        if qsp is None:
            resp_data = {
                "message": (
                    "Missing required query string parameters: userId and "
                    "attrName"
                )
            }
            resp_status = 400
            ...
            attr_name = qsp.get("attrName")

            # TODO: in the future bucket should be set as a required param. prefix
            #       will not be not required
            if None in (user_id, attr_name):
                resp_data = {
                    "message": (
                        "Missing required query string parameters: userId "
                        "and/or attrName"
                    )
                }
                resp_status = 400
            else:
                # TODO: need to ensure only public attrs are returned

                query_resp = ddb_table.get_item(
                    Key={"user_id": f"{user_id}"},
                    ProjectionExpression=f"{attr_name}",
                )

                print(f"Query response for single attribute: {query_resp}")
                ...
                if user_item is None:
                    ...
                    user_item = {}
                elif not len(user_item):
                    # attribute doesn't exist
                    # TODO: handle the hidden  attribute case
                    resp_status = 404
                    msg = (
                        "Attribute not found or caller doesn't have sufficient "
                        "permission to read user attribute."
                    )
                    user_item = {}
                else:
                    msg = f"Success"

                resp_data = {"message": msg, "attribute": user_item}
